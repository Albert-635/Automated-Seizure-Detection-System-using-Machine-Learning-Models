...
## Overview
NeuroGuard is an end-to-end Clinical Decision Support System (CDSS) designed to automate the detection of epileptic seizures from raw EEG recordings. Built to serve as a reliable "second opinion" for neurologists in Epilepsy Monitoring Units (EMUs) and ICUs, this suite processes continuous `.edf` files using a localized Hybrid AI inference engine and visualizes the results through a fully interactive, explainable clinical dashboard.
...
* **Clinical UI Dashboard (PyQt6):** Features real-time waveform rendering, an interactive timeline scrubber, and an "Explainable AI" 5-panel biomarker plot to visually justify the AI's detection zones.
...
1. **CHB-MIT Scalp EEG Database:** Pediatric patients (prolonged/evolving seizures).
2. **Siena Scalp EEG Database:** Adult patients (localized focal seizures).
